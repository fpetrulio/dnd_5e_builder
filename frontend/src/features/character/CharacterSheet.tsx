    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="opacity-60 text-sm mt-0.5">
          {state.race_id ? slugToLabel(state.race_id) : ''}{state.race_id ? ' · ' : ''}{classLabel}
        ...
        {tabs.map((t) => (
          <button
            ...
            className={cn(
              ...
              activeTab === t.id ? 'border-primary' : 'border-transparent opacity-50 hover:opacity-80',
            )}
      ...
      {activeTab === 'overview' && (
        <div className="space-y-4">
          ...
          {hasSpellcasting && (
            <div
              className="rounded-lg border p-3 flex gap-6"
              style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }}
            >
              <div>
          ...
          <div
            ...
            style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }}
          >
            <div className="text-xs font-medium opacity-50 mb-2">Passive Perception</div>
              ...
              >
                ...
                <div className="flex-1 border-l pl-4" style={{ borderColor: 'var(--color-border)' }}>
                  ...
                  <div className="flex items-center gap-1.5">
                    <div
                      className={cn('w-2 h-2 rounded-full', saveProf ? '' : 'opacity-20')}
                ...
                {Object.entries(computed.spell_slots).map(([level, count]) => (
                  ...
                  >
                    <div className="text-xs opacity-50">
                      {level.startsWith('pact') ? 'Pact' : `Level ${level}`}
                    </div>
                    <div className="font-bold flex items-center justify-center gap-1 mt-1">
                      <Dices size={12} className="opacity-50" />
                      {count}
                    </div>
                  </div>
                ))}
